...
bool CMyApp::Init()
{
	...
	// load texture
	m_waterTextureID = TextureFromFile("water.bmp");
	m_woodTextureID = TextureFromFile("wood.jpg");

	// load mesh
	//m_mesh = ObjParser::parse("Suzanne.obj");
	//m_mesh->initBuffers(); // must be done!
...
void CMyApp::DrawMesh()
{
	...
	for (int i = 0; i < 5; i++) {
		...
		float t = 2.0f * (float)M_PI / 5 * i
			+ SDL_GetTicks() / 3000.0f;
		...
		m_matWorld =		
		glm::rotate<float>(M_PI, glm::vec3(x, y, z))*
			glm::translate<float>(glm::vec3(x, y, z));
		/*glm::rotate<float>(M_PI*t1 / 2000.0f, glm::vec3(0, 0, 1))*
